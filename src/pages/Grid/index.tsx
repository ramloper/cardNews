            ...
            <div className='flex flex-row items-center justify-center gap-16'>
                {admin !== null && patchTitleStatus ? (
                    <input
                        ...
                        className='w-[200px] h-[40px] rounded-md border-2 border-gray-300 p-2'
                        value={titleValue}
                        onChange={(e) => setTitleValue(e.target.value)}
                    />
                ) : (
                    <h1 className='text-[26px] font-bold dark:text-white text-center'>{title?.title}</h1>
                )}
                {admin !== null && (
